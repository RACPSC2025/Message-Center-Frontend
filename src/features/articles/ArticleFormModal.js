...
export default function ArticleFormModal({
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // 🔹 Asegurar que id_requisito esté presente
      const dataToSend = { 
        ...formData,
        id_requisito: requisitoId || formData.id_requisito
      };
      ...
      if (response.status === 1) {
        ...
        toast.success(
          `${response.messages}. Se ha creado una notificación en el dashboard.`,
          {
            position: 'top-right',
            autoClose: 5000,
        ...
        // 🔹 Disparar evento para actualizar contador de mensajes
      ...
      toast.error('Error al crear artículo. Por favor intente nuevamente.', {
        position: 'top-right',
        autoClose: 5000
      });
    } finally {
