...
export default function TaskGroupList({ refreshTrigger }) {
  ...
  const selected_task_id = useSelector((state) => 
    selectFilterItemValue(state, 'task', 'selected_task_id')
  ) || {};

  const isSelected_task_id = useSelector((state) => 
    selectFilterItemValue(state, 'task', 'isSelected_task_id')
  ) || false;
    ...
    dispatch(fetchLogtaskList({ task_id: taskId })).then((data) => {
      if (data?.payload?.messages === 'Success') {
        const newLogtaskRows = data?.payload?.data.map((logtask) => ({
          ...
          closing_opportunity: !isNaN(parseInt(logtask.closing_opportunity_days)) ? parseInt(logtask.closing_opportunity_days) : '',
  ...
  const handleRefreshTaskData = (taskId) => {
    // Refrescar la lista completa de tareas para actualizar los contadores
    handleFetchListTaskNew(currentPage);
    
    // Si el accordion está expandido, también refrescar los logtasks
    if (accordionExpanded[taskId]) {
      handleFetchLogtaskList(taskId);
    }
  };

  // Nueva función para actualizar solo los contadores sin recargar todo
  const handleIncrementCommentCount = (taskId) => {
    // Actualización optimista: incrementar el contador inmediatamente
    setTasks((prevTasks) => 
      prevTasks.map((task) => 
        task.id === taskId 
          ? { ...task, comment_count: parseInt(task.comment_count || 0) + 1 }
          : task
      )
    );

    setTasksFilters((prevFilters) => 
      prevFilters.map((task) => 
        task.id === taskId 
          ? { ...task, comment_count: parseInt(task.comment_count || 0) + 1 }
          : task
      )
    ...
    // También actualizar logtaskRows si el accordion está expandido
    setLogtaskRows((prevRows) => 
      prevRows.map((logtask) => 
        logtask.task_id === taskId 
          ? { ...logtask, comments_logtask_count: parseInt(logtask.comments_logtask_count || 0) + 1 }
          : logtask
      )
    );
  };
  ...
  */
  ...
  useEffect(() => {
    ...
    if (tasksFiltersTemp.length > 0) {
      // Caso 1: Si hay una tarea seleccionada desde el message center
      if (isSelected_task_id && selected_task_id) {
        tasksFiltersTemp = tasks.filter((item) => item.id === selected_task_id);

        console.log("Filtro por tarea seleccionada desde message center: ", selected_task_id);
        console.log("tasksFiltersTemp: ", tasksFiltersTemp);
        
        // Limpiar el flag después de aplicar el filtro para evitar ciclos infinitos
        setTimeout(() => {
          //handleSetFilterItemValue('task', 'isSelected_task_id', false);
        }, 0);
        
        setTasksFilters(tasksFiltersTemp);
      } 
      // Caso 2: Aplicar filtros normales (keywords y status)
      else {
        if (actionKeyWords && actionKeyWords.trim() !== "") {
  ...
  }, [tasks, isSelected_task_id, selected_task_id, actionKeyWords, currentStatus]);

  /*
  useEffect(() => {
    let tasksFiltersTemp = tasks;

    // Si hay una tarea seleccionada desde el message center
    if (isSelected_task_id && selected_task_id) {
      tasksFiltersTemp = tasks.filter((item) => item.id === selected_task_id);
      
      // Opcional: Reset el flag después de aplicar el filtro
      // Esto evita que el filtro se mantenga activo indefinidamente
      // handleSetFilterItemValue('task', 'isSelected_task_id', false);
    }
    
    // ... resto de tu lógica de filtros existente ...
    
    // Actualizar el estado con las tareas filtradas
    //setFilteredTasks(tasksFiltersTemp);
    setTasksFilters(tasksFiltersTemp);

  }, [tasks, isSelected_task_id, selected_task_id]);
  */
        ...
        <Box className="max-h-[600px] overflow-y-auto m-[10px]">
          ...
          {Object.keys(selectedRow).length > 0 && (
            <EditEventDetailsDrawer
              ...
              onCommentAdded={() => handleRefreshTaskData(selectedRow.task_id)}
              //onCommentAdded={() => handleIncrementCommentCount(selectedRow.task_id)}
            />
          )}
          {Object.keys(selectedRow).length > 0 && (
