...
import { fetchUnreadMessagesCount } from '../stores/messages/unreadMessagesSlice'; // 🔹 Agregar import
...
function LayoutHeader() {
  ...
  // 🔹 Usar el nuevo contador de mensajes no leídos
  ...
  useEffect(() => {
    const handleMessageCreated = () => {
      ...
      if (user?.id_administradores) {
        //dispatch(fetchUnreadMessagesCount(user.id_administradores));
        dispatch(fetchUnreadMessagesCount('1'));
      }
    };
  ...
  useEffect(() => {
    ...
    // 🔹 Refrescar contador al cambiar de ruta
    refreshCount();
  }, [location]);
