...
import { evalWithIAresume } from '../../../stores/legal/fetchListLegalsSlice';
import { useDispatch } from 'react-redux';

export default function ResumeContentButton({ label = "Reemplazar" }) {
  ...
  const handleAIrequest = async (fullText) => {
    try {
      const action = await dispatch(evalWithIA(fullText));
      const payload = action?.payload ?? action;
      console.log('evalWithIA payload ->', payload);
  ...
  */
  ...
  const handleAIrequest = async (fullText) => {
    try {
      const action = await dispatch(evalWithIAresume(fullText));
      const newContent = action?.payload ?? '';

      console.log('Texto recibido desde IA →', newContent);

      if (!newContent || !newContent.trim()) {
        console.warn('La API no devolvió texto útil. No se reemplaza el contenido.');
        return;
      }

      // Insertar en el editor, respetando saltos de línea
      editor.update(() => {
        const root = $getRoot();
        root.clear();
  ...
  return (
    <>
      ...
      <Tooltip key={'replace_text'} title={t(label)}>
        ...
        >
          <DoneAll sx={{ width: '28px', height: '28px' }} />
        </Typography>
      </Tooltip>
    </>
  );
